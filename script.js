...
function switchLanguage(lang) {
    ...
    // 更新城市名称翻译显示
    const cityNameZhElements = document.querySelectorAll('.city-name-zh');
    cityNameZhElements.forEach(element => {
        if (lang === 'en') {
    ...
    if (timeLabel) {
        if (lang === 'zh') {
            timeLabel.textContent = '现在的韩国时间是';
        } else if (lang === 'kr') {
            timeLabel.textContent = '현재 한국 시간은';
        } else {
            timeLabel.textContent = 'Current Korea Time is';
        }
    }
    ...
    if (typeof updateDateDisplay === 'function') {
        const now = new Date();
        const seoulTime = new Date(now.getTime() + (9 * 60 * 60 * 1000) + (now.getTimezoneOffset() * 60 * 1000));
        updateDateDisplay(seoulTime);
    }
...
// 访客追踪功能
const API_BASE = 'http://112.124.111.161:5000'; // 后端服务器地址，后续有域名可替换这一行

function trackVisitor() {
    try {
        // 获取当前页面URL（带查询参数，方便区分页面）
        const pageUrl = window.location.pathname + window.location.search;
        
        // 发送访客信息到后端
        fetch(API_BASE + '/api/track-visitor', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then(data => {
            if (data.success) {
                ...
                console.log('访客记录失败:', data.message || '');
            }
        })
        .catch(error => {
            console.log('访客追踪失败:', error);
        });
    } catch (error) {
        console.log('访客追踪错误:', error);
    }
}
...
async function trackVisitor() {
    ...
    try {
        ...
        await fetch('https://krbeautyhub.com/api/track-visitor', {
...
(function() {
    ...
    function trackVisitor() {
        ...
        try {
            ...
            const dataStr = encodeURIComponent(JSON.stringify(trackData));

            img.src = 'http://112.124.111.161:5000/api/track-visitor-img?data=' + dataStr + '&t=' + new Date().getTime();
